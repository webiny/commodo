...
import getIndexes from '@commodo/fields-storage/getIndexes';
import hasIndex from '@commodo/fields-storage/hasIndex';
...
class DynamoDbDriver {
    ...
    async update({ model }) {
        const { id } = model;
        const item = await model.toStorage();
    ...
    async findOne({ model, options }) {
        return this._makeRequest({model, ...options}).then(data => {
    ...
    async find({ model, options}) {
       return this._makeRequest({model, ...options}).then(data => [data.Items, {}]);
    }

    async count({ model, options}) {
        return this._makeRequest({model, ...options}).then(data => data.Count);
    }
    ...
    _makeRequest({model, query, search, limit, before, after}) {
        const params = {
            TableName: this.getCollectionName(model),
        };
        ...
        if (Object.keys(mergedParams.attributeNames).length > 0) {
            params.ExpressionAttributeNames = mergedParams.attributeNames;
        }
        if (Object.keys(mergedParams.attributeValues).length > 0) {
        ...
        if (keyConditionParams.indexName && keyConditionParams.keyConditionExpression) {
            ...
            if (before) {
                params.ExclusiveStartKey = before;
            } else if (after) {
